...
use super::browser_scripting::{self, BrowserTarget, CursorPosition};
...
pub fn capture_text_and_frame(
    ...
    let browser_target = browser_scripting::detect_browser_target(app_bundle_id);
    ...
    if clipboard_mode {
        ...
        return CaptureResult {
            ...
            browser_target: None,  // Disable browser-specific features
        };
    }
    ...
    if let Some(target) = browser_target.as_ref() {
        log::info!(
            "Text capture: attempting JS capture for browser {:?}",
            target
        );
        if let Some(result) = browser_scripting::get_browser_text_and_cursor(target) {
            if result.found {
                log::info!(
                    "Text capture: JS succeeded, {} chars, cursor={:?}",
                    result.text.len(),
                    result.cursor
                );
                ...
                let element_frame = if initial_element_frame.is_none() {
                    ...
                    browser_scripting::get_browser_element_frame(target)
                } else {
            ...
            log::info!("Text capture: JS found no editable element, falling back to clipboard");
        } else {
    ...
    // IMPORTANT: Clear the browser target since JS-based live sync won't work.
    // This ensures we use clipboard-based restoration instead of failing JS calls
    log::info!(
        "JS capture failed, disabling browser live sync for this session"
    );
    ...
    let element_frame = if initial_element_frame.is_none() {
        if let Some(target) = browser_target.as_ref() {
            log::info!("Getting element frame via browser scripting");
            let browser_frame = browser_scripting::get_browser_element_frame(target);
            log::info!(
                "Browser scripting element frame: {:?}",
                browser_frame
                    .as_ref()
    ...
    let effective_browser_target = if browser_target.is_some() && is_address_bar {
        log::info!(
            "Browser address bar detected - disabling live sync to avoid updating web content"
        );
        None
    } else {
        // JS capture failed, so browser-specific live sync is unavailable.
        None
    };
    ...
    CaptureResult {
        ...
        browser_target: effective_browser_target,
    }
}
